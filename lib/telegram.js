...
const SYSTEM_PROMPT = [
  'You are a serious, practical assistant.',
  'Guidelines:',
  '- Be accurate, direct, and concise.',
  '- Prioritize useful answers over personality or flair.',
  '- Ask clarifying questions only when needed to avoid a wrong answer.',
  '- If you are uncertain, say so clearly and explain what information is missing.',
  '- Do not use whimsical language, roleplay, or decorative metaphors.',
  '- Keep responses focused on the user’s request.',
].join('\n');
const COMPACT_SYSTEM_PROMPT = [
  'Compact the conversation history into a concise factual summary.',
  'Preserve user preferences, key facts, unresolved tasks, and important decisions.',
  'Remove small talk, repetition, and details that are unlikely to matter later.',
...
function isTelegramHtmlParseError(error) {
  return /can't parse entities|parse entities/i.test(error?.message || '');
}
...
function getTelegramCommand(text) {
  ...
  return firstToken.slice(1).split('@', 1)[0].toLowerCase();
}
...
export async function sendMessageInChunks(chatId, text, options = {}) {
  ...
  for (let i = 0; i < chunks.length; i += 1) {
    ...
    try {
      await sendMessage(chatId, chunks[i], htmlOptions);
    } catch (error) {
      if (!isTelegramHtmlParseError(error)) {
        throw error;
      }

      const { parse_mode, ...plainTextOptions } = htmlOptions;
      logger.warn('Telegram rejected HTML formatting; retrying message as plain text.');
      await sendMessage(chatId, chunks[i], plainTextOptions);
    }
  }
}
...
async function processUserPrompt(chatId, userPrompt) {
  ...
  if (!rateLimit.allowed) {
    logger.warn(`Rate limit blocked chat ${chatId}: ${rateLimit.reason}`);
    await sendMessage(chatId, rateLimit.message);
    return;
  }
...
async function compactConversation(chatId) {
  const conversationHistory = await getConversation(chatId);
  if (conversationHistory.length === 0) {
    await sendMessage(chatId, 'No conversation history to compact.');
    return;
  }

  const rateLimit = await checkRateLimit(chatId);
  if (!rateLimit.allowed) {
    logger.warn(`Rate limit blocked compact for chat ${chatId}: ${rateLimit.reason}`);
    await sendMessage(chatId, rateLimit.message);
    return;
  }
  ...
  const summary = await getChatCompletion([
    ...
    ...conversationHistory,
    { role: 'user', content: 'Compact the conversation history now.' },
  ]);
  ...
  await sendMessage(chatId, 'Conversation history compacted.');
}
...
export async function handleUpdate(update) {
  ...
  try {
    ...
    if (command === 'start') {
      ...
      logger.info(`Compacted conversation history for chat ID ${chatId}.`);
    ...
    if (chatId) {
      try {
        ...
        logger.error('Unable to send error message to Telegram chat:', sendError);
      }
    }
    await notifyAdmin(
